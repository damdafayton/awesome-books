...
class Book {
    ...
    static removeBook(title2) {
        Book.books = Book.books.filter((book)=>book.title !== title2
        );
        store.setItem("books", JSON.stringify(Book.books));
    }
}
function addBook(e1) {
    ...
    if (Book.books.length % 2 === 1) li.classList.add("list-group-item-secondary");
    ...
    button.addEventListener("click", (e)=>{
        ...
        let title3 = liElem.querySelector(".title");
        title3 = title3.innerText;
        Book.removeBook(title3);
        liElem.remove();
        if (!Book.books.length) document.querySelector(".list").classList.remove("border", "border-dark", "border-2");
    });
    if (Book.books.length) {
        let count = 0;
        do {
            document.querySelector(".list").classList.add("border", "border-dark", "border-2");
            count += 1;
        }while (count === 1)
    }
